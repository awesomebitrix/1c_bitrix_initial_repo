...
class ImportOrgsRussia extends Command
{
	protected function configure()
	{
		$this
			->setName('totaldict:import_orgs_russia')
	...
	protected function execute(InputInterface $input, OutputInterface $output)
	{
		...
		while (($data = fgetcsv($fh)) !== false) {
			...
			if (strpos($data[13], ',')) {
				$explode = array_map('trim', explode(',', $data[13]));
				$arLoad['EMAIL'] = $explode[0];
				unset($explode[0]);
				$arLoad['PERSONAL_NOTES'] = implode(', ', $explode);
			} elseif (strpos($data[13], 'и')) {
				$explode = array_map('trim', explode('и', $data[13]));
				$arLoad['EMAIL'] = $explode[0];
				unset($explode[0]);
				$arLoad['PERSONAL_NOTES'] = implode(', ', $explode);
			} elseif (strpos($data[13], ';')) {
				$explode = array_map('trim', explode(';', $data[13]));
				$arLoad['EMAIL'] = $explode[0];
				unset($explode[0]);
				$arLoad['PERSONAL_NOTES'] = implode(', ', $explode);
